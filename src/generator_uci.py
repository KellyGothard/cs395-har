...
class DataGenerator(keras.utils.Sequence):
    ...
    def _gen(self, batch_idx):
        ...
        for i in batch_idx:
            ...
            # m = re.search(r"label([0-9])", self.filepaths[i])
            #labels.append(int(m.group(1)[-1])-1)
        ...
        if self.categorical:
            ...
            elif len(set(labels)) == 6:
                labels = np.array(labels) - 1
                batch_y = keras.utils.to_categorical(labels, num_classes=6, dtype='int8')
        else:
